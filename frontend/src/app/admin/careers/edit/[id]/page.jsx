...
export default function EditCareerJobPage() {
  ...
  useEffect(() => {
    async function loadJob() {
      ...
      setIsLoading(true);
      try {
        const res = await apiRequest(`/careers/jobs/${jobId}`, { method: "GET" }).catch(() => null);
        if (res?.success && res.data) {
          const j = res.data;
          setTitle(j.title || "");
          setTitleAr(j.titleAr || "");
          setDepartment(j.department || "Quality Assurance");
          if (j.department && !PRESET_DEPARTMENTS.includes(j.department)) {
            setIsCustomDept(true);
          }
          setDepartmentAr(j.departmentAr || "");
          setLocation(j.location || "");
          setLocationAr(j.locationAr || "");
          setJobType(j.jobType || "Full-Time");
          if (j.jobType && !PRESET_JOB_TYPES.includes(j.jobType)) {
            setIsCustomJobType(true);
          }
          setJobTypeAr(j.jobTypeAr || "");
          setOverview(j.overview || "");
          setOverviewAr(j.overviewAr || "");
          setResponsibilities(
          ...
          setResponsibilitiesAr(
            Array.isArray(j.responsibilitiesAr) && j.responsibilitiesAr.length > 0
              ? j.responsibilitiesAr
              : [""]
          );
          setRequirements(
          ...
          setRequirementsAr(
            Array.isArray(j.requirementsAr) && j.requirementsAr.length > 0
              ? j.requirementsAr
              : [""]
          );
          ...
          // Default fallback
          setTitle("Quality Control Engineer");
          setTitleAr("مهندس مراقبة الجودة");
          setDepartment("Quality Assurance");
          setDepartmentAr("ضمان الجودة");
          setLocation("Dubai, UAE");
          setLocationAr("دبي، الإمارات");
          setJobType("Full-Time");
          setJobTypeAr("دوام كامل");
          setOverview(
            "Ensure product quality and compliance with industry standards through rigorous testing, inspection, and data analysis."
          );
          setOverviewAr(
            "ضمان جودة المنتج والامتثال لمعايير الصناعة من خلال الاختبارات الصارمة والفحص وتحليل البيانات."
          );
          setResponsibilities([
          ...
          setResponsibilitiesAr([
            "إجراء الاختبارات المعملية الفيزيائية والكيميائية على المواد الخام والمنتجات النهائية.",
          ]);
          setRequirements([
            "Bachelor's Degree in Chemical Engineering or Applied Chemistry.",
          ]);
          setRequirementsAr([
            "درجة البكالوريوس في الهندسة الكيميائية أو الكيمياء التطبيقية.",
          ]);
        }
      } catch (err) {
        console.warn("Could not load job:", err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleAutoTranslate = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Backend translate error, using client fallback:", backendErr);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await apiRequest(`/careers/jobs/${jobId}`, {
        method: "PUT",
      ...
      setToastMsg("Job details updated successfully in MongoDB!");
      setTimeout(() => {
        router.push("/admin/careers");
      }, 1000);
    } catch (err) {
      setErrMsg(err.message || "Failed to update job posting.");
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[300px] text-gray-400">
        <Loader2 className="w-8 h-8 animate-spin text-gold-main mb-3" />
        <p className="font-heading font-bold text-sm">Loading Job Details...</p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Update position requirements, overview, or translations.
            </p>
          </div>
        </div>
