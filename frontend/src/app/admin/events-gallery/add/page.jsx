...
export default function AddEventPage() {
  ...
  // Make any image the First / Main Cover image
  ...
  const handleSubmit = async (targetStatus) => {
    ...
    if (!eventDate && !customDateDisplay) {
      setErrMsg("Please select an Event Date.");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        setToastMsg(`Event successfully ${finalStatus === "Published" ? "published" : "saved as draft"}!`);
        setTimeout(() => {
          router.push("/admin/events-gallery");
        }, 900);
      } else {
        setErrMsg(res.message || "Failed to create event. Please try again.");
      }
    } catch (err) {
      console.warn("API save error:", err);
      setErrMsg(err.message || "Failed to create event. Please try again.");
    } finally {
  ...
  const displayDesc = activeLang === "en" ? (enDescription || "Event description will appear here...") : (arDescription || enDescription || "تفاصيل الفعالية ستظهر هنا...");
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200/80 pb-4">
        ...
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-400 mb-1">
            ...
            <span>&gt;</span>
            <span className="text-gray-900 font-bold">Add New Event</span>
          </div>
        ...
        {/* Right: Actions (Preview Modal, Save Draft, Publish) */}
        <div className="flex items-center flex-wrap gap-2.5">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white border border-gray-200 hover:bg-gray-50 text-gray-800 text-xs font-heading font-bold shadow-2xs transition-all cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span className="text-[10.5px] text-gray-400 mt-1 block">
                  Floating badge pill on card (e.g. Jul 11, 2026)
                </span>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-gray-200/90 rounded-2xl p-5 sm:p-6 shadow-2xs space-y-5">
            ...
            {/* Hidden File Input (Accepts single or multiple files) */}
            <input
              type="file"
              ref={fileInputRef}
              accept="image/*"
