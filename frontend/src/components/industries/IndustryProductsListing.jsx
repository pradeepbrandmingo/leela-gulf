...
export default function IndustryProductsListing({
  ...
  return (
    <section
      ref={sectionRef}
      className="w-full bg-[var(--color-primary)] py-8 sm:py-12 lg:py-16 text-white relative"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-gray-400 font-semibold">Loading verified industry products...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-24 text-center bg-[#11131a]/60 rounded-3xl border border-[#252a38] p-6 sm:p-8">
            ...
            <h3 className="font-heading font-bold text-lg sm:text-xl text-white mb-2" style={{ fontWeight: 700 }}>
              {isRTL ? `لا توجد منتجات في ${activeIndustryInfo.ar} حالياً` : `No Products in ${activeIndustryInfo.name} Yet`}
            </h3>
            <p className="font-subheading text-gray-400 text-xs sm:text-sm max-w-md mb-6 leading-relaxed">
              {isRTL
                ? "نحن نوفر حلولاً مخصصة لهذا القطاع. يرجى التواصل مع فريق المشتريات لدينا لطلب مواصفات كيميائية خاصة."
                : "We source custom chemical grades for this sector. Contact our procurement desk for bulk quotation and tailored formulations."}
            </p>
            <Link
              href="/contact"
      ...
      {quoteProduct && (
        ...
        >
          ...
          >
            ...
            <LeadEnquiryForm
              sourcePage={`Industry Listing Quote - ${quoteProduct.title}`}
...
function IndustryProductCard({ product, isRTL, onQuoteRequest }) {
  return (
